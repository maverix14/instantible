...
if (!class_exists('daftplugInstantifyAdmin')) {
    class daftplugInstantifyAdmin {
        ...
        public function __construct($config, $defaultSettings, $daftplugInstantifyPwa, $daftplugInstantifyAmp, $daftplugInstantifyFbia) {
            ...
            $this->website = daftplugInstantify::getDomainFromUrl(trailingslashit(strtok(home_url('/', 'https'), '?')));
        ...
        public function addMenuPage() {
            $this->menuId = add_menu_page($this->menuTitle, (!$this->purchaseCode ?  $this->menuTitle.' <span class="awaiting-mod">1</span>' : $this->menuTitle), $this->capability, $this->slug, array($this, 'getPage'), $this->menuIcon);
        }
                        ...
                        <ul class="daftplugAdminMenu_list">
                            ...
                            foreach ($this->pages as $page) {
                                if (!empty($page['menuTitle'])) {
                                    ?>
                                    <li class="daftplugAdminMenu_item -<?php echo $page['id']; ?>">
                                        <a class="daftplugAdminMenu_link" href="#/<?php echo $page['id']; ?>/" data-page="<?php echo $page['id']; ?>">
